...
public interface EnglishExerciseDao extends JpaRepository<EnglishExerciseEntity, Integer> {
     ...
     * @return List&lt;String&gt;
     */
    @Query(value = "SELECT yiji FROM english_lib GROUP BY yiji", nativeQuery = true)
     ...
     * @return List&lt;String&gt;
     */
    @Query(value = "SELECT erji FROM english_lib WHERE yiji = :yiji GROUP BY erji", nativeQuery = true)
     ...
     */
    List<EnglishExerciseEntity> findByYijiAndErji(String yiji, String erji, Pageable pageable);
}
