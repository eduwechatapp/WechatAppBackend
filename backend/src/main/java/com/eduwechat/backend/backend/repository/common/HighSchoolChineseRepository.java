...
@Repository
public interface HighSchoolChineseRepository extends BaseCommonRepository<HighSchoolChinese, Integer> {
    /**
     * 根据二级标题分页查找
     * @param erji 二级标题
     * @param pageable 分页接口
    ...
    @Query(value = "select erji from gzyw where yiji = :yiji group by erji order by id", nativeQuery = true)
